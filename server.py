...
def derive_agronomy(
    ...
    for index in range(min(len(hourly["time"]), 72)):
        ...
        spray_flags.append(
            wind <= SPRAY_MAX_WIND_KMH
            and gust <= SPRAY_MAX_GUST_KMH
            and rain < SPRAY_MAX_RAIN_MM
            and rain_probability < SPRAY_MAX_RAIN_PROB_PCT
    ...
    smith_proxy = any(
        smith_days[index] and smith_days[index + 1]
        for index in range(max(len(smith_days) - 1, 0))
    )

    rain_last_7 = sum(value or 0 for value in history_daily["precipitation_sum"][-7:])
    rain_next_7 = sum(value or 0 for value in daily["precipitation_sum"][:7])
    mean_climate_rain = sum(value or 0 for value in climate_daily["precipitation_sum"]) / max(
        len(climate_daily["precipitation_sum"]),
        1,
    )
...
class AceWeatherHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/api/providers":
            self.send_json(
                {
                    "meteomaticsEnabled": get_meteomatics_credentials() is not None,
    ...
    def handle_search(self, query_string: str) -> None:
        ...
        if len(query) < 2:
            self.send_error_json(HTTPStatus.BAD_REQUEST, "Query must be at least 2 characters long.")
            return
        try:
            self.send_json(fetch_geocoding(query))
        except (urllib.error.URLError, urllib.error.HTTPError, json.JSONDecodeError, OSError) as exc:
            _log.error("Geocoding request failed: %s", exc)
            self.send_error_json(HTTPStatus.BAD_GATEWAY, "Location search is temporarily unavailable.")
    ...
    def handle_weather(self, query_string: str) -> None:
        ...
        try:
            ...
            self.send_error_json(HTTPStatus.BAD_REQUEST, "Latitude and longitude are required numeric values.")
            return
        ...
        try:
            ...
            self.send_error_json(HTTPStatus.BAD_REQUEST, "History days must be an integer.")
            return
        ...
        try:
            ...
            self.send_error_json(HTTPStatus.BAD_REQUEST, "Custom history dates must use YYYY-MM-DD format.")
            return

        try:
            payload = aggregate_weather(
                latitude,
                longitude,
                timezone,
                label,
            ...
            _log.error("Weather aggregation failed: %s", exc)
            self.send_error_json(HTTPStatus.BAD_GATEWAY, "Weather data is temporarily unavailable.")
