...
export function isAtlasPayload(value: unknown): value is AtlasPayload {
  ...
  ].every(isNumber) || !isText(headline.england_july_rain_context) || !isText(headline.england_august_rain_to_date)) return false;
  ...
  if (!isRecord(drought)
    ...
    || !isText(drought.groundwater_context)) return false;
